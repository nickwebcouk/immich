...
export class MetadataService extends BaseService {
  ...
  async handleMetadataExtraction(data: JobOf<JobName.AssetExtractMetadata>) {
    ...
    this.logger.verbose(`Checking live photo motion file for asset ${asset.id}: livePhotoVideoId=${asset.livePhotoVideoId ?? 'none'}`);
    try {
      if (asset.type === AssetType.Image && asset.livePhotoVideoId) {
        const motionAsset = await this.assetRepository.getById(asset.livePhotoVideoId);
        if (motionAsset) {
          const existsOnDisk = await this.storageRepository.checkFileExists(motionAsset.originalPath);
          if (existsOnDisk) {
            this.logger.debug(`Live photo motion file exists for motion asset ${motionAsset.id} at ${motionAsset.originalPath}`);
          } else {
            this.logger.log(`Live photo motion file missing for motion asset ${motionAsset.id}; regenerating from still asset ${asset.id}`);
            await this.applyMotionPhotos(asset as Asset, exifTags, dates, stats);
            this.logger.log(`Regeneration attempt completed for still asset ${asset.id}`);
          }
        } else {
          this.logger.debug(`Linked motion asset ${asset.livePhotoVideoId} not found in DB for asset ${asset.id}`);
        }
      }
    } catch (error: any) {
      this.logger.error(`Failed to verify/regenerate live photo motion file for ${asset.id}: ${error}`, error?.stack);
    }
